...
# install.packages("readxl") #설치하고 나서 #붙이는게 팁
library(readxl)
...
summary(gradexls$grade) # $ 뒤에 붙는 것은 변수명, 즉, grade 변수만 요약해서 보여주어라라 
# 실전 통계에서 이상치 확인할 때 도움

# R에서 class(특수하 형태의 숫자일 경우 어떻게 해석하는지를 보여줌) mode (문자, 숫자, 함수, 리스트 등)의 차이  
