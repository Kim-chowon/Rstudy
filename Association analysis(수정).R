...
basket.trans <- as(basket.trans, "transactions") # 바스켓 항목 하나를 transactions 으로 바꾸어 주겠다. 
basket.trans

inspect(basket.trans) # transactions을 보는 방법

# 연관성 규칙 도출(지지도0.1 이상, 신뢰도 0.8 이상인 연관성 규칙만 도출)
basket.apriori <- apriori(basket.trans, parameter= list(support=0.1,
                                      confidence=0.8))
inspect(basket.apriori) # 모든 가능성을 토대로 조합을 만들어 줌
?apriori
# lhs = 조건
# rhs = 결과
